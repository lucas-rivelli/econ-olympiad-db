...
function prepareLatexField(text) {
  // Strip path prefix from \includegraphics so \graphicspath can resolve it
...
function generateLatex(questions, includeSolutions) {
  ...
  const suffix = includeSolutions ? '_solutions' : '_questions';
  ...
  const blocks = questions.map((q, qi) => {
    const stmt = prepareLatexField(q.statement);
    ...
    if (q.choices && q.choices.length > 0) {
      const items = q.choices.map((c, ci) => {
        ...
        const text = prepareLatexField(c);
        return correct
          ? `  \\item \\colorbox{green!18}{\\strut\\textbf{(${letter}) ${text}}}\\;$\\checkmark$`
          : `  \\item ${text}`;
      }).join('\n');
      choicesLatex = `\n\\begin{enumerate}[label=(\\Alph*),leftmargin=2.2em,topsep=4pt,itemsep=2pt]\n${items}\n\\end{enumerate}`;
    }

    // Answer line for numeric / dissertation
    let answerLatex = '';
    if (includeSolutions && q.answer && q.type !== 'objective') {
      answerLatex = `\n\\par\\noindent\\textbf{Answer:} ${q.answer}\n`;
    }

    // Solution block
    let solutionLatex = '';
    if (includeSolutions && q.solution && q.solution.trim()) {
      solutionLatex = `\n\\begin{tcolorbox}[enhanced,breakable,title={\\textbf{Solution}},`
        + `colback=green!4!white,colframe=green!45!black,fonttitle=\\bfseries,`
        + `left=6pt,right=6pt,top=4pt,bottom=4pt]\n`
        + prepareLatexField(q.solution.trim())
        + `\n\\end{tcolorbox}`;
    }

    const meta = [q.source, q.year, q.round, q.topic, q.subtopic]
      .filter(Boolean).join(' · ');

    return `\\item \\textbf{${q.title || 'Question'}} \\hfill {\\footnotesize\\textit{${meta}}}\n\n`
      + stmt
      + choicesLatex
      + answerLatex
      + solutionLatex;
  }).join('\n\n\\bigskip\\noindent\\rule{\\linewidth}{0.3pt}\\bigskip\n\n');

  return `% ============================================================
% Generated by Econ Olympiad DB
% https://lucas-rivelli.github.io/econ-olympiad-db/
% Date: ${new Date().toLocaleDateString()}
%
% COMPILE:  pdflatex ${fname}${suffix}.tex
%           (requires: pdflatex + standard packages listed below)
%
% IMAGES:   If any question contains a figure, download the images/ folder
%           from https://github.com/lucas-rivelli/econ-olympiad-db
%           and place it in the same directory as this .tex file.
% ============================================================
...
\\begin{center}
  ...
  {\\large ${questions.length} Question${questions.length !== 1 ? 's' : ''}}\\\\[2pt]
  {\\small ${sources}}
\\end{center}
...
\\begin{enumerate}[leftmargin=*,label=\\textbf{\\arabic*.},itemsep=2.2em]
